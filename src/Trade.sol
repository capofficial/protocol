...
contract Trade is ITrade {
    ...
    function getExecutableOrderIds()
        public
        view
        returns (uint256[] memory orderIdsToExecute)
    {
    ...
    function _executeOrder(
        IStore.Order memory order,
        uint256 price,
        address keeper
    ) internal {
        // Check for existing position
        IStore.Position memory position = store.getPosition(
            order.user,
            order.market
        );

        bool doAdd = !order.isReduceOnly &&
            (position.size == 0 || order.isLong == position.isLong);
    ...
    function _increasePosition(
        ...
        uint256 keeperFee = (fee * store.getKeeperFeeShare()) / BPS_DIVIDER;
        fee -= keeperFee;
    ...
    function _decreasePosition(
        ...
        if (order.isReduceOnly) {
            ...
            store.incrementBalance(
                order.user,
                (order.fee * remainingOrderSize) / order.size
            );
        } else {
    ...
    function getLiquidatableUsers()
        public
        view
        returns (address[] memory usersToLiquidate)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (marginLevel < store.getMinimumMarginLevel()) {
                _users[j] = user;
                j++;
            }
        }
    ...
    function liquidateUsers() external {
        ...
        for (uint256 i = 0; i < usersToLiquidate.length; i++) {
            ...
            for (uint256 j = 0; j < positions.length; j++) {
                ...
                uint256 fee = (position.size * market.fee) / BPS_DIVIDER;
                uint256 liquidatorFee = (fee * store.getKeeperFeeShare()) /
                    BPS_DIVIDER;
                fee -= liquidatorFee;
                ...
                pool.creditTraderLoss(
                    user,
                    position.market,
                    position.margin - fee - liquidatorFee
                );
    ...
    function getAccruedFunding(string memory market, uint256 intervals)
        public
        view
        returns (int256)
    {
        if (intervals == 0) {
            intervals =
                (block.timestamp - store.getFundingLastUpdated(market)) /
                store.getFundingInterval();
        }
